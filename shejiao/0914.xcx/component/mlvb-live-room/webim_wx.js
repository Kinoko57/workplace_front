...
module.exports = function () {
    ...
    (function (webim) {
        ...
        // 已登录初始化
        var _wechatlogin = function (loginInfo, listeners, options, cbOk, cbErr) {
            ...
            ConnManager.apiCall(SRV_NAME.PROFILE, "portrait_set", {
                ...
                function (resp) {
                    for (var i in options.ProfileItem) {
                        ...
                        if (profile.Tag == 'Tag_Profile_IM_Image') {
                            ctx.identifierAvater = profile.Value; //更新昵称
                            break;
                        }
                    }
                    if (cbOk) cbOk(resp);
        ...
        Msg.Elem.GroupTip = function (opType, opUserId, opUserName, opUserAvatar, groupId, groupName, userIdList) {
            this.opType = opType; //操作类型
            this.opUserId = opUserId; //操作者id
            this.opUserName = opUserName;
            this.opUserAvatar = opUserAvatar;
        ...
        Msg.Elem.GroupTip.prototype.getopUserName = function () {
            return this.opUserName;
        };
        Msg.Elem.GroupTip.prototype.getopUserAvatar = function () {
            return this.opUserAvatar;
        };
        ...
        var MsgManager = new function () {
            ...
            var handlerC2cEventMsg = function (notify) {
                ...
                switch (subType) {
                    case C2C_EVENT_SUB_TYPE.READED: //已读通知
                        break;
                    case C2C_EVENT_SUB_TYPE.KICKOUT: //顶号通知
                        // 调用util的方法，提示退出
                        break;
                    default:
                ...
                // stopPolling = true;
                //回调 onMsgReadCallback
                if(notify.KickoutMsgNotify!=undefined){
                    // onC2cEventCallbacks[subType](notify.KickoutMsgNotify);
                    // util.imKickOut(notify.KickoutMsgNotify)
                    // console.log(util)
                    imKickOut(notify.KickoutMsgNotify)
                }
            ...
            this.longPolling = function (cbOk, cbErr) {
                ...
                function doPolling() {
                    proto_longPolling(opts, function (resp) {
                        for (var i in resp.EventArray) {
                            ...
                            switch (e.Event) {
                                ...
                                case LONG_POLLINNG_EVENT_TYPE.C2C_COMMON: //c2c消息通知
                                    ...
                                    break;
                                case LONG_POLLINNG_EVENT_TYPE.C2C_EVENT: //c2c已读消息通知 实际是IM账号被顶的通知
                        ...
                        //log.error(err);
                        if(err.ErrorCode==91101){
                            // util.imKickOut('小程序顶小程序了')
                            // console.log(util)
                            imKickOut('小程序顶小程序了')
                        }
                        // console.log(err)
            ...
            this.bigGroupLongPolling = function (cbOk, cbErr) {
                ...
                proto_bigGroupLongPolling(opts, function (resp) {
                    ...
                    if (err.ErrorCode != longPollingKickedErrorCode) {
                        //登出
                        // console.log(longPollingKickedErrorCode)
            ...
            var handlerGroupMsg = function (msgInfo, isSyncGroupMsgs, isAddMsgFlag, isFinished) {
                ...
                if (msgInfo.MsgBody) {
                    if (msgInfo.MsgBody.MsgMemberExtraInfo) {
                        fromAccountNick = msgInfo.MsgBody.MsgMemberExtraInfo[0].NickName;
                        headUrl = msgInfo.MsgBody.MsgMemberExtraInfo[0].ImageUrl;
                    }
                }
                if (msgInfo.From_Account == ctx.identifier) { //当前用户发送的消息
                    isSendMsg = true;
                    ...
                    switch (msgType) {
                        ...
                        case MSG_ELEMENT_TYPE.GROUP_TIP:
                            ...
                            msgContent = new Msg.Elem.GroupTip(
                                ...
                                msgBody.MsgContent.MsgOperatorMemberExtraInfo.NickName,
                                msgBody.MsgContent.MsgOperatorMemberExtraInfo.ImageUrl,
                                group_id,
        ...
        //登录
        webim.weChatlogin = webim.init = function (loginInfo, listeners, opts, cbOk, cbErr) {
            // console.log("初始化连接状态回调函数")
            MsgManager.resetLongPollingInfo();
            //初始化连接状态回调函数
            ConnManager.init(listeners.onConnNotify, cbOk, cbErr);
            // console.log('回调了吗')
            //设置ie9以下浏览器jsonp回调
            if (listeners.jsonpCallback) jsonpCallback = listeners.jsonpCallback;
            _wechatlogin(loginInfo, listeners, opts, cbOk, cbErr);
        };
        //登出
        //需要传长轮询id
