    ...
    <div class="level_bg">
      <div class="fifter">
        <my-tabs
          ...
          :add="btnPermission('onlinedmcourseagentlevel_create')"
          ...
          <!-- <search-fifter :add="btnPermission('onlinedmcourseagentlevel_create')" :module-name="'onlinedmcourseagentlevel'" :fifter="true" :add-text="'创建等级'" @search="fifterSearch" @addAction="addProxyLabel"/> -->
      </div>
      <div class="level_info">
        ...
        <div class="info_total">
          <div class="data_total">
            ...
            <el-button v-if="btnPermission('onlinedmcourseagentlevel_batchdown')" plain size="mini" @click="batchstop('stop')">批量停用</el-button>
            <el-button v-if="btnPermission('onlinedmcourseagentlevel_batchabolish')" plain size="mini" @click="batchstop('remover')">批量作废</el-button>
            <el-button v-if="btnPermission('onlinedmcourseagentlevel_batchup')" plain size="mini" @click="batchstop('start')">批量启用</el-button>
          </div>
        </div>
        <el-table v-loading="loading" ref="multipleTable" :data="tableData" :max-height="tableHeight" border row-key="id" style="width: 100%" tooltip-effect="light" @sort-change="fifterTable" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="操作" align="center" min-width="200">
            <template slot-scope="scope">
              <el-button v-if="btnPermission('onlinedmcourseagentlevel_edit')" type="text" size="small" @click="editProxyLabel(scope.row)">编辑</el-button>
              <el-button v-if="btnPermission('onlinedmcourseagentlevel_banned')&&scope.row.status==2" type="text" size="small" @click="changeStates(scope.row,'1')">启用</el-button>
              <el-button v-if="btnPermission('onlinedmcourseagentlevel_banned')&&scope.row.status==1" type="text" size="small" @click="changeStates(scope.row,'2')">停用</el-button>
              <el-button v-if="btnPermission('onlinedmcourseagentlevel_cancellation')" type="text" size="small" @click="deleteSelect(scope.row,'99')">作废</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="info_page">
      ...
      <el-scrollbar class="fixed_scroll">
        <el-form
          ...
          <el-form-item label="备注：">
            <el-input :rows="4" v-model.trim="levelForm.remarks" maxlength="120" type="textarea" style="width:500px" placeholder="请输入备注,最大长度为120个字" />
          </el-form-item>
        </el-form>
      </el-scrollbar>
...
export default {
  ...
  data() {
    return {
      page_name: 'onlinedmcourseagentlevel',
      module_name: 'olsdm',
  ...
  methods: {
    ...
    getList() {
      ...
      getonlinecourseagentlevel(that.searchInfo).then(res => {
        ...
        if (res.code === 200) {
          console.log(res)
          that.tableData = res.data.onlinedmcourseagentlevel
