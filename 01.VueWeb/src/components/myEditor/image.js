...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		//   console.log(document.getElementsByClassName('ke-input-text')[0])
          let file = document.getElementsByClassName('ke-upload-file')[0].files[0]
          if (file==null) {
            Message({
              type: 'warning',
              message: "请选择图片"
		...
		//   zipFormData.append('filename','files' );
		...
		var uploadbutton = K.uploadbutton({
			...
			afterUpload : function(data) {
				...
				if (data.code === 200) {
					var url = data.data.path + data.data.data[0].pathname;
					console.log(url)
					if (formatUploadUrl) {
