    ...
    <div class="type_bg">
      <div class="fifter">
        <my-tabs
          ...
          :add="btnPermission('onlinecourse_create')"
      ...
      <div class="type_info">
        <div class="info_total">
          <div class="data_total">
            ...
            <el-button v-if="btnPermission('onlinecourse_cancellations')" plain size="mini" @click="batchstop('remover')">批量作废</el-button>
            <el-button v-if="btnPermission('onlinecourse_banneds')" plain size="mini" @click="batchstop('stop')">批量停用</el-button>
            <el-button v-if="btnPermission('onlinecourse_bannedsup')" plain size="mini" @click="batchstop('start')">批量启用</el-button>
          </div>
        </div>
        <el-table
          v-loading="loading"
          ...
          <el-table-column prop="createdDate" label="创建时间" width="300" min-width="100" show-overflow-tooltip/>
          ...
          <el-table-column v-if="tableData.length>0" label="操作" min-width="200" align="center" fixed="right">
            <template slot-scope="scope">
              <el-button v-if="btnPermission('onlinecourse_edit')" type="text" size="small" @click="editProxyLabel(scope.row)">编辑</el-button>
              <el-button v-if="btnPermission('onlinecourse_banned')&&scope.row.status==2" type="text" size="small" @click="changeStates(scope.row,'1')">启用</el-button>
              <el-button v-if="btnPermission('onlinecourse_banned')&&scope.row.status==1" type="text" size="small" @click="changeStates(scope.row,'2')">停用</el-button>
              <el-button v-if="btnPermission('onlinecourse_cancellation')" type="text" size="small" @click="deleteStates(scope.row,'99')">作废</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="info_page">
      ...
      <el-scrollbar class="fixed_scroll">
        ...
        <el-form
          ...
          <el-form-item label="上级分类：" prop="lastleavel">
            <el-select v-model="typeForm.lastleavel" placeholder="请选择上级分类,若不选则默认顶级分类" style="width:500px">
  ...
  methods: {
    ...
    getList() {
      ...
      getonlinecourse(that.searchInfo).then(res => {
        ...
        if (res.code === 200) {
          // console.log(res)
          that.tableData = res.data.onlinecourse
