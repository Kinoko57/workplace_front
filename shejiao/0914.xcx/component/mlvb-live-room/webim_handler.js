...
var selToID, loginInfo, MemberList, accountMode, accountType, sdkAppID, avChatRoomId, selType, selToID, selSess, selSessHeadUrl;
//监听大群新消息（普通，点赞，提示，红包）
...
function handleGroupMessage(msg, textMessageCallback, sketchpadDataCallback) {
    ...
    if (msg.fromAccountNick == '@TIM#SYSTEM') {
        // console.log(msg)
        // // 先获取当前账号的昵称 msg.elems[0].content.opUserId
        let tag_list = [
            "Tag_Profile_IM_Nick",
            "Tag_Profile_IM_Image"
        ];
        let options = {
            'To_Account': [msg.elems[0].content.opUserId],
        ...
        webim.getProfilePortrait(options,
            function (res) {
                ...
                // console.log(res)
                msg.fromAccount = res.UserProfileItem[0].ProfileItem[0].Value
                msg.fromAccountNick = res.UserProfileItem[0].ProfileItem[1].Value
                textMessageCallback && textMessageCallback(showMsg(msg));
            },
            function (ret) {
                // console.log('获取失败', ret);
                textMessageCallback && textMessageCallback(showMsg(msg));
            })
    } else {
        textMessageCallback && textMessageCallback(showMsg(msg));
    ...
    for (var i in elems) {
        ...
        content.memberInfoList = MemberList==undefined?[]:MemberList
        if (!content) {
...
function sdkLogin(userInfo, listeners, options, avChatRoomId, callback, callbackOptions) {
    ...
    webim.login(userInfo, listeners, options,
        function (identifierNick) {
            ...
            // console.log("sdk登录-webim登录成功")
            loginInfo = userInfo;
            // console.log(userInfo)
            // console.log('获取缓存的用户信息', wx.getStorageSync('userInfo'))
            // console.log("设置进入用户的信息")
            setProfilePortrait({
                'ProfileItem': [{
                    ...
                    },
                    {
                        ...
                        "Value": wx.getStorageSync('userInfo').wx_image
                    }
                ]
            }, function (ret) {
                if (ret) {
...
// 微信小程序sdk登录
function sdkWechatLogin(userInfo, listeners, options, avChatRoomId, callback, callbackOptions) {
    //web sdk 登录
    webim.login(userInfo, listeners, options,
        function (identifierNick) {
            //identifierNick为登录用户昵称(没有设置时，为帐号)，无登录态时为空
...
// 初始化监听weChatlogin
function initWechat(userInfo, listeners, options, avChatRoomId, callback, callbackOptions) {
    // console.log("初始化监听weChatlogin")
    //web sdk 登录
    webim.weChatlogin(userInfo, listeners, options,
        function (identifierNick) {
            //identifierNick为登录用户昵称(没有设置时，为帐号)，无登录态时为空
            webim.Log.info('webim登录成功');
            // console.log("sdk登录-webim登录成功")
...
// 根据id获取个人的昵称和头像
function getProfilePortrait(options, data, callback) {
    webim.getProfilePortrait(options,
        function (res) {
            // 处理结果 UserProfileItem
            let list = res.UserProfileItem
            let result_list = []
            result_list = list.map(item => {
                return {
                    Member_Account: item.To_Account,
                    identifierNick: item.ProfileItem[0].Value,
                    identifierAvatar: item.ProfileItem[1].Value,
                    Role: ''
                }
            })
            result_list = result_list.map(item => {
                data.map(key => {
                    if (item.Member_Account == key.Member_Account) {
                        item.Role = key.Role
                    }
                })
                return item
            })
            callback && callback(result_list);
        },
        function (ret) {
            // console.log('获取失败', ret);
            callback && callback(ret);
...
// 获取群详细信息
function getBigMember(groupId, callback, callbackOptions) {
    webim.getGroupInfo({
            GroupIdList: [groupId],
            GroupBaseInfoFilter: [
                "Type",
        ...
        function (resp) {
            ...
            if (resp.ActionStatus === 'OK') {
                // console.log('获取群详细信息成功');
                // 轮询修改群成员列表的信息
                selToID = groupId;
                let memberlist = resp.GroupInfo[0].MemberList
                let search_member_ids = []
                search_member_ids = memberlist.map(item => {
                ...
                // ];
                // let options = {
                //     'To_Account': search_member_ids.slice(0,4),
                //     'LastStandardSequence': 0,
                //     'TagList': tag_list,
                //     'memberlist': memberlist.slice(0,4)
                // };
                // getProfilePortrait(
                //     options,
                //     memberlist,
                //     function (res) {
                //         // console.log(res)
                //         resp.GroupInfo[0].MemberList = res
                //         let pusher = {}
                ...
                //             }
                //         }
                //         resp.GroupInfo[0].pushers = [pusher]
                //         MemberList = res
                //         callback && callback({
                //             errCode: 200,
                //             callback: callbackOptions,
...
function showMsg(msg) {
    ...
    switch (subType) {
        ...
        case webim.GROUP_MSG_SUB_TYPE.TIP: //群提示消息
            // content = "[通知]" + convertMsgtoHtml(msg);
            // console.log('群提示消息')
            // console.log(msg)
            content = convertMsgtoHtml(msg);
            break;
...
function convertGroupTipMsgToHtml(content) {
    ...
    var opType, opUserId,opUserName,opUserAvatar, userIdList;
    var memberCount;
    opType = content.getOpType(); //群提示消息类型（操作类型）
    opUserId = content.getOpUserId(); //操作人id
    // console.log(content.getMemberInfoList())
    opUserName = content.getopUserName();//名称
    opUserAvatar = content.getopUserAvatar();//头像名称
    ...
    switch (opType) {
        case webim.GROUP_TIP_TYPE.JOIN: //加入群
            ...
            // text = '{"cmd":"notifyPusherChange","data":{"userID":"' + opUserId + '"}}'
            text = '{"cmd":"CustomCmdMsg","data":{"cmd":"2","nickName":"' + opUserName + '","headPic":"' + opUserAvatar + '"}}'
...
function getOneName(userID){
    ...
    webim.getProfilePortrait(options,
        function (res) {
            ...
            // console.log(res)
            let obj ={
                nickName:res.UserProfileItem[0].ProfileItem[0].Value,
                headPic:res.UserProfileItem[0].ProfileItem[1].Value,
            }
            return obj
        },
        function (ret) {
...
//发送消息(群点赞消息)
function sendGroupLoveMsg(options) {
...
module.exports = {
    ...
    sdkLogin: sdkLogin,
    sdkWechatLogin: sdkWechatLogin,
    initWechat: initWechat,// 初始化监听
    createBigGroup: createBigGroup,
    applyJoinBigGroup: applyJoinBigGroup, // 进入大群
    getBigMember: getBigMember, // 获取群详细信息
    getOneName: getOneName, //获取指定账号的详细信息
    showMsg: showMsg,
