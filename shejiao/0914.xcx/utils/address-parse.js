  ...
  defaultData.forEach(province => {
    if (province.child) {
      province.child.forEach(city => {
        if (city.local_name !== '其他') {
          if (!mCity[city.local_name]) {
            mCity[city.local_name] = [];
          }
          mCity[city.local_name].push({
            p: { name: province.local_name, id: province.id },
            c: { name: city.local_name, id: city.id },
            a: city.child || []
          });
        }
        if (city.child) {
...
/*   address = address.replace(/\r\n/g, ' ').replace(/\n/g, ' ').replace(/\t/g, ' ');
  ...
  */
  ...
  //   if (el.name.indexOf(parse.province) == 0 && parse.province != '') {
  //     parse.province = el.name
  //   }
  // })

  // zipCode.forEach(provice => {
  //     if (parse.province.indexOf(provice.name) == 0 && parse.province!='') {
...
function detail_parse_forward(address) {
  ...
  const cityKey = ['布依族苗族自治州', '苗族侗族自治州', '自治州', '州', '市', '县','区'];
  for (let i in defaultData) {
    const province = defaultData[i];
    let index = address.indexOf(province.local_name);
    if (index > -1) {
      if (index > 0) {
        //省份不是在第一位，在省份之前的字段识别为名称
        parse.name = address.substr(0, index).trim();
      }
      parse.province = { name: province.local_name, id: province.id} ;
      address = address.substr(index + province.local_name.length);
      ...
      for (let j in province.child) {
        const city = province.child[j];
        index = address.indexOf(city.local_name);
        if (index > -1 && index < 3) {
          parse.city = { name: city.local_name, id: city.id };
          address = address.substr(index + city.local_name.length);
          ...
          if (city.child) {
            for (let k in city.child) {
              const area = city.child[k];
              console.log(city,"kldsfj")
              index = address.indexOf(area.local_name);
              if (index > -1 && index < 3) {
                parse.area = area;
                address = address.substr(index + area.local_name.length);
                break;
              }
            }
          }
