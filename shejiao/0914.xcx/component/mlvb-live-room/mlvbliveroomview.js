...
Component({
    ...
    methods: {
        ...
        enter() {
            ...
            liveroom.enterRoom({
                ...
                success: function (ret) {
                    console.info('进入房间 成功: ', ret)
                    self.data.audience.url = self.data.roomurl;
                    self.data.audience.mixUrl = self.data.roomurl;
                    // 
                    if (ret.pushers && ret.pushers.length > 0) {
                        self.data.audience.pusherName = ret.pushers[0].identifierNick;
                        self.data.audience.pusherAvatar = ret.pushers[0].identifierAvatar;
                        self.data.audience.pusherID = ret.pushers[0].Member_Account;
                        // self.data.audience.accelerateUrl = ret.pushers[0].accelerateURL;
                    } else {
                        console.error('缺少加速流');
                    }
                    ret.url = self.data.roomurl;
                    ret.mixUrl = self.data.roomurl;
                    // console.log(self.data.audience)
                    //{ "cmd":"C2CCustomMsg", "data":{ userName: self.data.audience.pusherName, userAvatar:self.data.audience.pusherAvatar, "roomID":"XXX", "cmd":"xx", msg:"xx" } }
                    // 给主播发送进房通知
                    ...
                    liveroom.sendRoomTextMsg({
                        data: {
                            msg: ret.accountInfo.push_key + "_dz"
                        },
                        success: () => {},
                        fail: (e) => {
                            // console.log("发送消息失败: ", e)
        ...
        exit: function () {
            // console.log('mlvbliveroomview，exit() called')
            // console.log("退出直播间")
            let options = {
    ...
    attached: function () {
        ...
        var {
            userID,
            userName
        } = liveroom.getAccountInfo();
        self.data.isCaster = self.data.role == 'anchor';
        // console.log(self.data.roomurl)
        self.setData({
