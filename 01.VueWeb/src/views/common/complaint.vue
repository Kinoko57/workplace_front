<template>
  <div>
    <div class="welfare_bg">
      <div class="fifter">
        <my-tabs :page-name="page_name" :module-name="module_name" :add="btnPermission('complain_create')"
          :other="btnPermission('complainlevel_browse')" :other-text="'投诉类型'" :active-name="searchInfo.mode"
      ...
      <div class="welfare_info">
        ...
        <el-table v-loading="loading" ref="table" :data="tableData" :max-height="tableHeight" border row-key="id"
          ...
          <el-table-column label="投诉类型" prop="processName">
            <template slot-scope="scope">
              <span>{{ scope.row.processName }}</span>
            </template>
          </el-table-column>
          <!-- <el-table-column prop="complainNumber" label="投诉编号" min-width="100" show-overflow-tooltip /> -->
          <el-table-column prop="sourceName" label="投诉主题" min-width="150" show-overflow-tooltip />
          ...
          <el-table-column label="操作" min-width="200" align="center" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button v-if="(searchInfo.mode=='finishdeal')" type="text" size="small" @click="viewDetail(scope.row)">
                回访</el-button>
              <el-button v-if="searchInfo.mode=='founding'&&btnPermission('complain_edit')"
        ...
        <el-scrollbar v-show="(modelType=='new'||modelType=='edit')&&modelShow" class="fixed_scroll over_scroll">
          ...
          <el-form v-show="(modelType=='new'||modelType=='edit')&&modelShow" ref="complaintForm" :model="complaintForm"
            ...
            <el-form-item label="投诉标题：" prop="title">
              <el-input v-model.trim="complaintForm.title" style="width:500px" placeholder="请输入投诉标题" />
            </el-form-item>
            <el-form-item label="投诉类型：" prop="leaveId">
            ...
            <el-form-item label="登记时间：">
              <el-date-picker v-model="complaintForm.registrantDate" :default-time="false" type="datetime"
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (this.btnPermission('complain_all') == false) {
        this.typeList = [{
            label: '待我审核',
            name: 'wait'
          },
    ...
    methods: {
      ...
      getList() {
        ...
        getComplaintInfo(this.searchInfo).then(res => {
          ...
          that.loading = false
          if (res.code === 200) {
            that.tableData = res.data.copworkflows
      ...
      viewDetail(data) {
        this.$router.push({
          path: '/office/complaintDetail?id=' + data.id + '&type=' + this.searchInfo.mode
        })
      },
      // 审批申请单
      showModel(data, type) {
