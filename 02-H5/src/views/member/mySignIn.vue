    ...
    <div class='sign_in'>
      <div class='sign_body'>
        <div class='sign_banner'>
          <img :src='imgPath+"member/sign_bg_02.jpg"' />
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getSignDays().then(res => {
        if (res.isSuccess) {
          ...
          console.log(that.signTime)
          console.log(new Date().getDate())
          console.log(that.signTime.includes(new Date().getDate()))
          if (that.signTime.includes(new Date().getDate())) {
            that.isSign = true
          }
        }
      })
    ...
    goScore() {
      this.$router.togo({
        path: '/proList?type=key&virtual_id=&price=asc&value='
      })
    },
    getValue(e) {
      console.log(e)
