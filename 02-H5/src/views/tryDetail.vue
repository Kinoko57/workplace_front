  ...
  <div>
    ...
    <div v-if='isShow'>
      ...
      <div class='try_btn' v-if='hiddenFix'>
        ...
        <mt-button class='application' @click='showModel' v-if='proInfo.num>proInfo.count&&proInfo.help_list.length==proInfo.help_num&&proInfo.help_list.length>0&&proInfo.status==1&&form_share==0'>邀请成功</mt-button>
      ...
      <div class='goIndex' @click='zipImg(proInfo.id)' v-if="proInfo.is_help==0">
      ...
      <div class='share_info' v-if='showShare' catchtouchmove="ture">
        ...
        <div class='share_people'>
          <div class='share_img_list'>
            ...
            <div class='share_img' v-for='(item,index) in proInfo.help_num-proInfo.help_list.length' :key='index'>
              <img :src="imgPath+'com/add_img.png'"/>
            </div>
          </div>
        ...
        <div class='share_btn'>
          <mt-button @click='zipImg(proInfo.id)' v-if='form_share==0&&proInfo.help_list.length<proInfo.help_num'>邀请助力</mt-button>
          <mt-button @click='zipImg(proInfo.id)' v-if='form_share==1&&proInfo.help_list.length<proInfo.help_num'>更多助力</mt-button>
          ...
          <div v-for='(item,index) in specList' :key='index' :hidden='!hasSpec'>
            ...
            <div class='spec_type'>
              <mt-button size='small' @click="choseType(item.tid,item.pid)" v-for="(item,index) in item.list" :key='index'
                id='item.tid' data-pid='item.pid' :class="item.ischose? 'button_click': ''">{{item.name}}</mt-button>
            </div>
          </div>
...
export default {
  ...
  mounted() {
    ...
    if (storage.getItem('isLogin') === 'false' || storage.getItem('isLogin') === false) {
  ...
  methods: {
    ...
    share(id, img) {
      ...
      console.log('是否是助力' + this.proInfo.is_help)
      let shareUrl = 'pages/tryOutdetail/tryOutdetail?id=' + id + '&form_share=' + this.proInfo.is_help + '&help_id=' + this.help_id + '&uid=' + storage.getItem('token').split(';')[0].split('=')[1]
      let linkURL = ''
      let shareTitle = ''
      if (this.proInfo.is_help == 1) {
        shareTitle = '[邀请助力]' + that.proInfo.name
      } else {
        shareTitle = that.proInfo.name
      }
