        ...
        <div class="follow_scroll_short">
          ...
          <div class="info">
            <ul>
              ...
              <!-- <li><span>经销商等级：</span><span>{{ proxyInfo.name }}</span></li> -->
              <li><span>客服：</span><span>{{ proxyInfo.salesId }}</span></li>
              <!-- <li><span>客服号：</span><span>{{ proxyInfo.serviceId }}</span></li> -->
              <li class="width_line"><span>地址：</span><span>{{ proxyInfo.address }}</span></li>
            <!-- <li class="width_line"><span>备注：</span><span>无</span></li> -->
            </ul>
          </div>
          ...
          <div class="info_title">
            <span>客户类型:</span>
            <template>
              <el-select v-model="assignInfo.levleId" placeholder="请选择客户标签">
                <el-option
            ...
            <ol>
              <li v-for="(item,index) in assignList" :key="index">
                ...
                <span v-if ="item.action=='assign'">
                  ...
                  <span v-if="showFiles[index][0].name!=''">
                    {{ showFiles[index].name }}
                    <span>{{ item.date }} ，由</span> <span class="font_bold">{{ item.actor }}</span><span> 添加了跟进 </span><span>，内容为 </span><span class="font_bold">{{ item.comment.content }}</span><span>，附件为 </span><span v-for="(item,index) of showFiles[index]" :key="index" ><a :href="fileUrl+item.url" download="" style="color:#409EFF;text-decoration: underline;display:block;font-weight:normal;">{{ item.name }}</a> </span>
                  </span>
                </span>
                ...
                <span v-if ="item.action=='agentingore'"><span>{{ item.date }} ，由</span> <span class="font_bold">{{ item.actor }}</span><span> 忽略。</span></span>
...
<script>
import {
  gthFollowagent, gthSaveagentFollow
...
export default {
  ...
  methods: {
    ...
    getList() {
      const that = this
      gthFollowagent(this.proxyId).then(res => {
        if (res.code === 200 || res.code === '200') {
          ...
          that.assignInfo.levleId = that.proxyInfo.levleId
          ...
          for (var i = 0; i < that.assignList.length; i++) {
            ...
            if (that.assignList[i].action === 'assign') {
              if (that.assignList[i].comment.file !== null && that.assignList[i].comment.file !== 'null' && that.assignList[i].comment.file !== '') {
                ...
                that.assignList[i].file = [{ name: '', path: '' }]
              }
            }
            that.$forceUpdate()
          }
    ...
    onSubmit() {
      ...
      if (this.assignInfo.content === '') {
        ...
        if (this.filesArr.length > 0) {
          this.assignInfo.file = JSON.stringify(this.filesArr)
        }
        gthSaveagentFollow(this.proxyId, this.assignInfo).then(res => {
