  ...
  <div>
    ...
    <div class="sale" v-if="isShow">
      <div class="sale_body">
        ...
        <div class="sale_tab_show">
          <div class="sale_tab">
            <div
              :class="item.id==choseId?'sale_nav sale_active':'sale_nav'"
        ...
        <div class="sale_list" >
          <div class="sale_pro" v-for="(item,index) in saleProList" :key="index">
            ...
            <div class="pro_info">
              <div class="pro_title" @click="goPro('flash',item.goods_id)">{{item.name}}</div>
              <div class="pro_progress">
                <div class="prog">
                  <div
                    :style="{width:item.count_num/item.store*100+'%'}"
                    v-if="item.count_num/item.store*100<100"
                  ></div>
              ...
              <div class="pro_spec">
                <div class="pro_price">
                  <div class="pro_inter_img" v-if="item.deal_type!=2">
                    <span
                      class="peo_nowprice"
                      v-if="item.deal_type==1||item.deal_type==3"
                    >￥{{item.price_sell}}<span style="color:#ff4e79;font-size:0.23rem" v-if="isMember"> / 赚{{item.back_rebate}}</span></span>
                  ...
                  <span
                    class="pro_integral"
                    v-if="item.deal_type!=1&&item.deal_type!=2&&item.deal_type!=3"
                  >{{item.point}}</span>
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center;">
                  ...
                  <div class="pro_icon">
                    <mt-button
                      size="small"
                      @click="goPro('flash',item.goods_id)"
                      v-if="status==0"
                      class="active"
                    >立即抢购</mt-button>
                    <mt-button
        ...
        <div class="list_bottom" v-if="loading&&total>0&&saleProList.length>0">—————— 我是有底线的 ——————</div>
...
import { InfiniteScroll, Indicator, Button, Lazyload } from 'mint-ui'
import { getSaleTime, getSaleList } from '@/api/index'
      ...
      getSaleList(info).then(res => {
        if (res.isSuccess) {
          if (res.data.list.rows !== null && res.data.list.rows.length > 0) {
            ...
            that.total = res.data.list.total
            that.loading = false
            if (res.data.list.rows < that.pagesize) {
              that.loading = true
            }
          } else {
            that.loading = true
