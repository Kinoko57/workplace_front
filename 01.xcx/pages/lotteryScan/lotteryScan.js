...
Page({
  ...
  gowinList: function() {
    if (this.data.btnDisabled == '') {
      ...
      //获取当前滚动距离
      var that = this
      util.queryRequest('?m=ewmlottery&a=info', {
        page: that.data.page,
        type: 0,
        limit: 20,
        genre: 2 //0：全部中奖列表  1：大转盘    2超级转盘
      }, 'GET', function(res) {
        if (res.isSuccess) {
  ...
  winPro: function(e) {
    ...
    if (this.data.allowbind) {
      ...
      util.queryRequest('?m=listexchange&a=add', {
        ...
        if (res.isSuccess) {
          wx.navigateTo({
            url: '../sureOrder/sureOrder?id=' + id + '&addType=3&genre=2'
          })
        } else {
          that.setData({
            allowbind: true
    ...
    if (that.data.btnDisabled == '') {
      ...
      if (util.isLogin()) {
        if (wx.getStorageSync("isbind")) {
          util.queryRequest('?m=ewmlottery&a=cj', {}, 'GET', function(res) {
            if (res.isSuccess) {
              that.setData({
                msg: res.msg
              })
          ...
          wx.reLaunch({
            url: '../bind/bind?visit=7' // 已授权未绑定手机号跳转到绑定手机号后跳转回来
          })
        }
      } else {
        that.setData({
  ...
  onShow: function() {
    ...
    var that = this
    util.queryRequest('?m=activity&a=ewm_lottery_index', { code: that.data.code}, 'POST', function(res) {
      if (res.isSuccess) {
        ...
        that.setData({
          ...
          list_bg_top: res.data.background_image4
    ...
    util.queryRequest('?m=activity&a=ewm_winning', {
