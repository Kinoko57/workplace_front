...
const conf = {
  ...
  calculateDays(year, month, curDate) {
    ...
    util.queryRequest('?m=myinfo&a=my_footprint', {
      ...
      date: day == undefined ? '' : (year + '-' + (month < 10 ? '0' + month : month) + '-' + (day < 10 ? '0' + day : day)),
      page:1,
      pagesize:100
    }, 'POST', function(res) {
      console.log(res.data.time_list)
      wx.hideLoading()
      if (res.isSuccess) {
        ...
        console.log(res.data.foot_list)
        var pages = getCurrentPages()
        var prevPage = pages[pages.length - 1] //上一个页面
        prevPage.data.total = res.data.foot_list.total
        prevPage.data.footList = res.data.foot_list.rows
        prevPage.setData({
          total: prevPage.data.total,
          footList: prevPage.data.footList
        })
        const tmp = {
          'calendar.days': days,
        };
...
export default (config = {}) => {
  ...
  conf.jumpToToday.call(self);
  const functionArray = ['tapDayItem', 'choosePrevMonth', 'chooseYearAndMonth', 'chooseNextMonth', 'calendarTouchstart', 'chooseYearAndMonth', 'pickerChange', 'tapPickerBtn'];
