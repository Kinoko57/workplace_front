<template>
  <div>
    <div class="course_bg">
      <div class="fifter">
        <!-- <search-fifter
          :add="btnPermission('onlinedmstudy_create')"
          :add-text="'创建排课'"
          :module-name="'onlinedmstudy'"
          :export-show="btnPermission('onlinedmstudy_export')"
          :fifter="true"
      ...
      <div class="course_info">
        ...
        <div class="info_total">
          <div class="data_total">
            ...
            <el-button v-if="btnPermission('onlinedmstudy_cancellations')" plain size="mini" @click="batchstop('remover')">批量作废</el-button>
        ...
        <el-table v-loading="loading" ref="table" :data="tableData" :max-height="tableHeight" border row-key="id" tooltip-effect="light" @sort-change="fifterTable" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="排课日期" min-width="120">
            <template slot-scope="scope">
              <span class="text_color">{{ scope.row.uploadDate }}</span>
            </template>
          </el-table-column>
          <el-table-column label="排课分类" prop="studyLeavelName" min-width="120" show-overflow-tooltip/>
          <el-table-column label="摇篮群" min-width="120" show-overflow-tooltip>
            <template slot-scope="scope">
              <span class="text_color">{{ scope.row.cradlegroupIdName }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="techerIdName" label="讲师" min-width="100" show-overflow-tooltip/>
          ...
          <el-table-column v-if="tableData.length>0" label="操作" min-width="200" align="center" fixed="right">
            <template slot-scope="scope">
              <el-button v-if="btnPermission('onlinedmstudy_edit')" type="text" size="small" @click="editCourse(scope.row)">编辑</el-button>
              <el-button v-if="btnPermission('onlinedmstudy_setresult')" :disabled="isdisabled && scope.row.status==1" type="text" size="small" @click="viewCourse(scope.row)">结果</el-button>
              <el-button v-if="btnPermission('onlinedmstudy_updatestatus')" type="text" size="small" @click="changeStates(scope.row,'99')">作废</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="info_page">
      ...
      <el-scrollbar v-if="modelType!='view'&&modelShow" class="fixed_scroll">
        <el-form
          ...
          <el-form-item v-if="courseForm.cradlegroupId!=''" label="讲师：" prop="techerId">
            <el-select v-model="courseForm.techerId" filterable clearable placeholder="请选择讲师名称" style="width:500px">
...
export default {
  ...
  data() {
    ...
    return {
      page_name: 'onlinedmstudy',
      module_name: 'olsdm',
      choseTotal: 0,
      uploadUrl: '',
      moduleType: '/olsdm/onlinedmstudy-export',
      loading: true,
      vieinfo: {},
      tableData: [],
      optionsList: [],
    ...
    getList() {
      ...
      getonlinestudy(that.searchInfo).then(res => {
        ...
        if (res.code === 200) {
          ...
          that.tableData = res.data.onlinedmstudy
