...
Page({
    ...
    if (delList.length == 0) {
      ...
      wx.showModal({
        ...
        success: function (res) {
          if (res.confirm) {
            util.queryRequest('/app/footOther/cancelShare', {
              c_id: delList.join(','),
              type:that.data.type
  ...
  getList: function () {
    var that = this
    util.queryRequest('app/footOther/shareIndex', {
      page: that.data.page,
  ...
  async isLeft(list) {
    ...
    for (const item of list) {
      col1SH <= col2SH ? col1.push(item) : col2.push(item); //判断两边高度，来觉得添加到那边
  ...
  getBoxHeight(col1, col2) { //获取左右两边高度
    return new Promise((resolve, reject) => {
      this.setData({
        ...
        query.exec((res) => {
          if(res[0]!=null){
            col1SH = res[0].height; //获取左边列表的高度
          }
          if(res[1]!=null){
            col2SH = res[1].height; //获取右边列表的高度
          }
          resolve();
        });
      });
