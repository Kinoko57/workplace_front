...
Page({
  ...
  onLoad: function () {
    console.log("onLoad")
    console.log(wx.getStorageSync('refresh'),"初始")
    // if (wx.getStorageSync('refresh') == undefined || wx.getStorageSync('refresh') == 0) {
      ...
      console.log(wx.getStorageSync('now_activity_id'))
      if (wx.getStorageSync('now_activity_id') != '' && wx.getStorageSync('now_activity_id') != undefined) {
        this.setData({
          activityId: wx.getStorageSync('now_activity_id')
        })
        wx.setStorageSync('activity_id', wx.getStorageSync('now_activity_id'))
        // 根据id获取动态列表
        this.getList()
      }
      // 获取分类列表
      ...
      setTimeout(() => { //3S后隐藏骨架屏
        ...
        query1.exec(function (res) {
          ...
          console.log(app.globalData.useWindowHeightFoot)
          that.setData({
            scrollHeight: app.globalData.useWindowHeightFoot - res[0].height
          })
        })
      }, 500)
    // } else {
    //   console.log("执行onShow把")
    // }
  },
  ...
  onShow: function () {
    ...
    // 重新获取列表数据，
    // 1、如果发生了点赞或者删除点赞的行为
    console.log(wx.getStorageSync('refresh'))
    if (wx.getStorageSync('refresh') != undefined && wx.getStorageSync('refresh') == 1) {
      // console.log("1、如果发生了点赞或者删除点赞的行为,主动刷新分类和动态")
      // this.setData({
      //   col1: [],
      //   col2: [],
      //   articleList: [],
      ...
      // 2、切换了种草分类
      console.log("2、切换了种草分类")
      console.log(wx.getStorageSync('activity_id'),"值1")
      console.log(wx.getStorageSync('now_activity_id'),"值")
      if (wx.getStorageSync('now_activity_id') != wx.getStorageSync('activity_id')) {
        this.setData({
          activityId: wx.getStorageSync('now_activity_id')
        })
        console.log()
      ...
      // console.log(getApp().globalData.article_type)
