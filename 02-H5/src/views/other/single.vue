      ...
      <div class='vip_body_list'>
            ...
            <div class='vip_pro' v-for="(item,index) in vipList" :key="index">
              ...
              <div class='pro_info'>
                ...
                <div class='pro_spec'>
                  <div class='pro_price'>
                    <div>
                      <div class='pro_nowprice'>
                        <span v-if='item.deal_type==1||item.deal_type==3'>￥{{item.price_sell}}<span style="color:#ff4e79;font-size:0.23rem" v-if="isMember"> / 赚{{item.back_rebate}}</span></span>
                        <span v-if='item.deal_type>=4'>￥{{item.price_sell}}<span style="color:#ff4e79;font-size:0.23rem" v-if="isMember"> / 赚{{item.back_rebate}}</span></span>
                      </div>
                  ...
                  <!-- 加入购物车 -->
                  <div class='pro_icon'>
                    <i class='iconfont icon-car' @click='addCar(item.product_id,item.deal_type,item.price_sell,item.buy_count)'></i>
                  </div>
                </div>
              </div>
            </div>
            <div class='order-foots' v-if='vipList.length==0'>
              <img :src="imgPath+'pro/null_1.png'" mode='widthFix'/>
              <!-- <span>暂无数据</span> -->
            </div>
          </div>
        ...
        <div class='car_edit_more'>
          <div class='allCheck'>
            ...
            <span class='car_tips' v-if='difference>0'>自营商品还差{{difference}}元，可享包邮</span>
            <span class='car_tips' v-else>自营商品已满{{full_amount}}元享受包邮</span>
          </div>
...
export default {
  ...
  mounted() {
    ...
    this.difference = this.$store.state.home.carDifference
      ...
      if (num > 0) {
        ...
        addCarofPro(info).then(res => {
          if (res.isSuccess) {
            ...
            that.difference = parseFloat(that.difference) * 1000 - parseFloat(price) * 1000
...
<style scoped lang="less">
  ...
  .fifter_brand_more .chose_text {
    border-bottom: 2/100rem solid #ff4e79;
  }
