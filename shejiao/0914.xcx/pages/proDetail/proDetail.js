...
Page({
  ...
  data: {
    ...
    specShow: false, // 规格弹框
    specNum: 1, // 购买数量
    product_keys_names_ids: [], // 精简版的属性值，属性名，货号的组合数组
  ...
  goBuy(e) {
    if (util.isLogin()){
      wx.navigateTo({
        url: '../payment/payment?type=0&goods_id=' + this.data.proInfo.goods_id+ '&act_id=0' + '&product_id=' + this.data.proInfo.product_id + '&num=' + this.data.specNum,
      })
    }else{
      wx.navigateTo({
        url: '../login/login',
  ...
  choseSpec(e) {
    ...
    if (e.currentTarget.dataset.disable == false) {
      ...
      let chose_spec_id = e.currentTarget.dataset.parentid + ':' + e.currentTarget.dataset.id
  ...
  getProductInfo(arr) {
    ...
    for (let i = 0; i < this.data.productsList.length; i++) {
      if (JSON.stringify(arr) == JSON.stringify(this.data.productsList[i].key)) {
        ...
        this.data.proInfo.buy_price = this.data.productsList[i].buy_price
        ...
        this.data.proInfo.purchase_price = this.data.productsList[i].purchase_price
        ...
        this.data.proInfo.product_status = this.data.productsList[i].is_up
        flag = true
      }
      if (flag) {
        break
  ...
  getDetail() {
    let that = this
    util.queryRequest('/app/goods/get', {
      ...
      if (res.code == 200) {
        ...
        for (let i in res.data.attr_product) {
          ...
          // res.data.attr_product[i].imgList = JSON.parse(res.data.attr_product[i].img)
        ...
        console.log("更改后的规格数组")
        console.log(res.data.attr_data)
        ...
        that.setData({
          ...
          product_keys_names_ids: arr,
          choseIds: static_spec_list,
