    ...
    <div class="group_bg">
      <div class="fifter">
        <my-tabs
          ...
          :add="btnPermission('cradlegroup_create')"
      ...
      <div class="group_info">
        <div class="info_total">
          <div class="data_total">
            ...
            <el-button v-if="btnPermission('cradlegroup_cancellations')" plain size="mini" @click="batchstop('remover')">批量作废</el-button>
          </div>
          <div class="export_btn">
            <search-fifter
              :download-url="downloadUrl"
              :module-name="'cradlegroup'"
              :upload-url="uploadUrl"
              :import-show="btnPermission('cradlegroup_import')"
              :export-show="btnPermission('cradlegroup_export')"
        ...
        <el-table
          ...
          <el-table-column label="群组名称" prop="name" show-overflow-tooltip min-width="200"/>
          ...
          <el-table-column prop="createdDate" label="创建时间" show-overflow-tooltip min-width="200"/>
          <el-table-column label="操作" min-width="160" align="center">
            <template slot-scope="scope">
              <el-button v-if="btnPermission('cradlegroup_edit')" type="text" size="small" @click="editCradlegroup(scope.row)">编辑</el-button>
              <el-button v-if="btnPermission('cradlegroup_cancellation')" type="text" size="small" @click="changeStates(scope.row,'99')">作废</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="info_page">
...
export default {
  ...
  data() {
    return {
      page_name: 'cradlegroup',
      module_name: 'ols',
      choseTotal: 0,
      typeList: [{ label: '全部摇篮群', name: 'all' }],
      submitting: false,
      loading: true,
      uploadUrl: '',
      downloadUrl: '',
      modelType: '',
      moduleType: '/ols/cradlegroup-export',
      tableData: [],
      searchInfo: {
        mode: 'all', // 类型
        search: '', // 搜索关键字
  ...
  mounted() {
    this.uploadUrl = process.env.BASE_API + 'ols/cradlegroup-import.json'
    this.downloadUrl = process.env.BASE_API + 'data/download/摇篮群模板.xls'
    this.getList()
  },
  created() {
  ...
  methods: {
    batchstop(type) {
      ...
      if (that.selectArr.length > 0) {
        ...
        that.$confirm('摇篮群一旦作废，不可以恢复,确定要批量“作废”吗？', '提示', {
          ...
          cradlebatchCancel(obj).then(res => {
    ...
    getList() {
      ...
      getCradlegroupInfo(that.searchInfo).then(res => {
        ...
        if (res.code === 200) {
          // console.log(res)
          that.tableData = res.data.cradlegroup
