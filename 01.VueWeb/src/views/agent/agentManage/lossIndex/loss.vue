      ...
      <div class="intent_info">
        <div class="info_total">
          <div class="data_total">
            <span class="total_text">共{{ total }}个经销商</span>
            <!-- <el-button plain size="mini">加入黑名单</el-button> -->
          </div>
          <div class="export_btn">
            <search-fifter :export-show="btnPermission('agent_export')" :export-type="moduleType" :module-name="'agent'" :export-name="'流失经销商'"/>
          </div>
        </div>
        <el-table
          v-loading="loading"
          ...
          <el-table-column label="操作" width="200" align="center" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button
                type="text"
                size="small"
                @click="goFllow(scope.row)">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="info_page">
      ...
      <el-scrollbar class="fixed_scroll">
        <el-form
          ...
          <el-form-item label="指派：" prop="salesId">
            <el-select v-model="resaleForm.salesId" filterable placeholder="请选择指派对象" style="width:500px">
              <el-option v-for="(item,index) in selectList" :label="item.label" :value="item.value" :key="index"/>
            </el-select>
          </el-form-item>
          <el-form-item label="指派原因：" prop="remarks">
            <el-input :rows="4" v-model="resaleForm.remarks" type="textarea" placeholder="请输人内容,最大长度为120个字" style="width:500px"/>
          </el-form-item>
        </el-form>
      </el-scrollbar>
...
export default {
  ...
  methods: {
    goFllow(data) {
      this.isShow = false
      this.$router.push({
        path: '/agent/lossIndex/lossfollow?id=' + data.id + '&choseType=' + this.searchInfo.mode
      })
    },
