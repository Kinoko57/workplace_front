  ...
  <div>
    <div class="member_bg">
      ...
      <div class="member_info">
        ...
        <el-table v-loading="loading" v-if="searchInfo.mode=='all'" ref="tabel" :data="tableData" :max-height="tableHeight" border style="width: 100%" tooltip-effect="light" @sort-change="fifterTable">
          <el-table-column label="批次名称" prop="batch" min-width="120" show-overflow-tooltip/>
          <el-table-column label="群人员" prop="number" min-width="120"/>
          <el-table-column label="创建时间" prop="createdDate" min-width="140" show-overflow-tooltip/>
          <el-table-column v-if="tableData.length>0" label="操作" min-width="100" align="center">
            <template slot-scope="scope">
              <el-button v-if="btnPermission('cradlegroupmember_browse')" type="text" size="small" @click="showMemberList(scope.row)">成员管理</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-table
...
export default {
  ...
  mounted() {
    console.log(this.btnPermission('cradlegroupmember_getmember'))
    if (this.btnPermission('cradlegroupmember_getmember') == false) {
      this.typeList = [{
        label: '全部批次',
        name: 'all'
      }]
      this.searchInfo.mode = 'all'
    } else if (this.btnPermission('cradlegroupmember_getmember') == true) {
  ...
  methods: {
    ...
    getList() {
      ...
      if (that.searchInfo.mode == 'all') {
        getcradlegrouplist(that.searchInfo).then(res => {
          ...
          if (res.code === 200) {
          // console.log(res)
            that.tableData = res.data.cradlegrouplist
    ...
    choseType(value) {
      ...
      if (this.searchInfo.mode == 'all') {
        this.module_name = 'ols'
        this.page_name = 'cradlegrouplist'
        this.isOpen = false
      } else {
        this.module_name = 'ols'
        this.page_name = 'cradlegroupmember'
        this.isOpen = true
      }
      this.loading = true
      this.getList()
    ...
    showMemberList(data) {
      console.log(1)
      this.$router.push({
        path: '/online/memberlist?id=' + data.id
