...
const conf = {
  data: {
    actionList:[
      {id:'0', actionIcon:'icon-qingkong',actionTips:'删除'},
      {id:'1',actionIcon:'icon-coll',actionTips:'收藏'},
    ],
    ...
    if (this.data.allchose == 0) {
      for (var i = 0; i < list.length; i++) {
        for (let j = 0; j < list[i].length; j++) {
          ...
          this.data.delList.push(list[i][j].goods_id)
        }
      }
      this.setData({
        allchose: 1
    ...
    if (that.data.delList.length <= 0) {
      ...
      wx.showModal({
        ...
        success: function(res) {
          if (res.confirm) {
            util.queryRequest('/app/track/remove', {
              ids: that.data.delList.join(",")
  ...
  getFootList: function(year, mon, day) {
    ...
    var that = this
    that.data.page = 1
    util.queryRequest('/app/track/index', {
