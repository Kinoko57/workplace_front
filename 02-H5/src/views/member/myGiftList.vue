    ...
    <div class='my_gift'>
      ...
      <div class='gift_info' v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="30">
        <div class='info' v-for='(item,index) in giftList' :key='index'>
          ...
          <button size='mini' class='btn_red' v-if='item.get_status==0' @click='winPro(item.pid,item.id,item.genre)'>点击领取</button>
        </div>
      ...
      if (this.$store.state.home.isbind === 'false' || this.$store.state.home.isbind === false) {
        ...
        if (this.allowbind) {
          ...
          addCarofGift(info).then(res => {
            if (res.isSuccess) {
              that.$router.togo({
                path: '/sureOrder?id=' + id + '&addType=3&proType=exchange&genre=' + genre
              })
            } else {
              that.allowbind = true
              Toast({
