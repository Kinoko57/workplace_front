        ...
        <transition :name="fadeXType">
          <div :key="showDate.monthStr" class="e-calendar-monthday-wrapper">
            <div class="e-calendar-monthday-row" v-for="(row,index) in rows" :key='index'>
              <span v-for="(day,index) in row" :key='index' class="e-calendar-monthday-row-day" @click="selectDay(day)"
                :class="{'active': day.hasSign, 'disabled': day.disabled, 'pointer': day.value !== ''}">
                <img :src="imgPath+'member/chose_date.jpg'" class="before" v-if="day.beforeChoose&&day.hasSign&&day.week!=1">
                <span v-text="day.value" class="e-calendar-monthday-row-day-value"></span>
                <img :src="imgPath+'member/chose_date.jpg'" class="after" v-if="day.afterChoose&&day.hasSign&&day.week!=7">
...
export default {
    ...
    rows() {
      ...
      for (let i = 1; i <= months; i += 1) {
        ...
        if (i === 1 && weekValue !== 1) {
          ...
          this.addRowDayValue(row, i, this.signDays.includes(i), weekValue)
        } else {
          this.addRowDayValue(row, i, this.signDays.includes(i), weekValue)
      ...
      for (let i = 0; i < result.length; i++) {
        for (let j = 0; j < result[i].length; j++) {
          if (result[i][j].value != '') {
            if (j > 0) {
              if (result[i][j - 1].hasSign != undefined && result[i][j - 1].hasSign) {
                result[i][j].beforeChoose = true
              }
            }
          }
        }
  ...
  methods: {
    ...
    addRowDayValue(row, i, isSign, weekValue) {
      const value = {
        value: i,
        hasSign: false,
        beforeChoose: false,
