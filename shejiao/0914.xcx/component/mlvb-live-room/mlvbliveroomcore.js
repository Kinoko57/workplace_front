...
function login(options) {
	...
	accountInfo.push_key = options.data.push_key;
	accountInfo.userName = wx.getStorageSync('userInfo').nickname;
	accountInfo.userAvatar = wx.getStorageSync('userInfo').wx_image;
	request({
		url: 'login',
		params: {
			accountType: '0',
...
function loginIM(options) {
	...
	var listeners = {
		...
		"onMsgNotify": function (newMsgList) { //监听新消息(私聊(包括普通消息和全员推送消息)，普通群(非直播聊天室)消息)事件，必填
			webimhandler.onMsgNotify(newMsgList, function (msg) {
				// console.log("core.js--监听新消息(私聊(包括普通消息和全员推送消息)，普通群(非直播聊天室)消息)事件，必填-----获取msg")
				// console.log(msgs)
				// recvC2CMsg(msg);
			});
		},
	...
	// console.log('登录之前获取用户信息')
	loginInfo.identifierNick = wx.getStorageSync('userInfo').nickname
	loginInfo.identifierAvatar = wx.getStorageSync('userInfo').wx_image
	// console.log(loginInfo)
	// 判断IM是否登录
	if (wx.getStorageSync('imInfo') != '' && wx.getStorageSync('imInfo') != undefined) {
		// 直接进入房间
		// console.log('webim已经登录成功，直接进入房间');
		webimhandler.initWechat(loginInfo, listeners, others, 0, afterLoginIM, options);
...
// 小程序登录时登录IM
function loginWechatIM(options) {
	// 初始化设置参数
	webimhandler.init({
		accountMode: accountInfo.accountMode,
		accountType: '0',
	...
	if (accountInfo.accountMode == 1) { //托管模式
		webimhandler.sdkWechatLogin(loginInfo, listeners, others, 0, afterLoginIM, options);
	} else { //独立模式
		//sdk登录
		webimhandler.sdkWechatLogin(loginInfo, listeners, others, 0, afterLoginIM, options);
	}
...
function receiveMsg(msg) {
	...
	time = h + ':' + m + ':' + s;
	msg.time = time;
	// msg.userID = accountInfo.userID;
	// msg.userName = accountInfo.userName;
	// msg.userAvatar = accountInfo.userAvatar;
	// console.log(msg)
	if (msg.fromAccountNick == '@TIM#SYSTEM') {
		...
		if (contentObj.cmd == 'CustomTextMsg') {
			...
			msg.userName = contentObj.data.nickName == undefined ? contentObj.data.userName : contentObj.data.nickName;
			msg.userAvatar = contentObj.data.headPic == undefined ? contentObj.data.userAvatar : contentObj.data.headPic;
			msg.cmd = contentObj.data.cmd;
			var content = '';
			for (var i = 1; i < newContent.length; i++) {
				if (i == newContent.length - 1)
			...
			msg.content = content;
			if (contentObj.data.cmd == '5') {
				// 弹幕消息
				msg.content = contentObj.data.msg;
			}
			if(contentObj.data.cmd == '7'){
				// 主播信息，观众列表，观众人数
				// console.log('mlvbliveroomcore --- 主播信息，观众列表，观众人数')
				msg.content = JSON.parse(contentObj.data.msg);
			}
			// console.log(msg)
...
function notifyPusherChange(options) {
	// var customMsg = {
	// 	cmd: "3",
	// 	data: {
	...
	// console.log("发送退群消息")
	webimhandler.sendCustomMsg({
		data: '{"cmd":"CustomCmdMsg","data":{"cmd":"3","userName":"' + options.userName + '","userAvatar":"' + options.userAvatar + '"}}',
		text: 'notify'
	}, function () {
		options.success && options.success();
	});
...
function sendRoomTextMsg(options) {
	...
	webimhandler.sendCustomMsg({
		data: '{"cmd":"CustomTextMsg","data":{"type":"0","nickName":"' + accountInfo.userName + '","headPic":"' + accountInfo.userAvatar + '"}}',
...
function sendRoomLOVEMsg(options) {
	if (!options || !options.data.msg || !options.data.msg.replace(/^\s*|\s*$/g, '')) {
		// console.log('sendRoomTextMsg参数错误', options);
		options.fail && options.fail({
			errCode: -9,
	...
	webimhandler.sendGroupLoveMsg({
		data: '{"cmd":"CustomCmdMsg","data":{"type":"2","cmd":"4","userName":"' + accountInfo.userName + '","userAvatar":"' + accountInfo.userAvatar + '"}}',
		text: options.data.msg
	}, function () {
		options.success && options.success();
	});
...
// 发送自定义消息，发送进房消息
function sendRoomCustomMsg(options) {
	if (!options || !options.data.msg || !options.data.msg.replace(/^\s*|\s*$/g, '')) {
		// console.log('sendRoomTextMsg参数错误', options);
		options.fail && options.fail({
			errCode: -9,
	...
	webimhandler.sendCustomMsg({
		data: '{"cmd":"CustomCmdMsg","data":{"cmd":"2","userName":"' + accountInfo.userName + '","userAvatar":"' + accountInfo.userAvatar + '"}}',
		text: '进房通知'
	}, function () {
		options.success && options.success();
	});
...
function proto_enterRoom(options) {
	// console.log('proto_enterRoom进入房间-开始IM: ', roomInfo.roomID);
	webimhandler.applyJoinBigGroup(roomInfo.roomID, afterJoinBigGroup, {
		success: function (ret) {
			// 实际获取列表信息是在getMemberInfo里
			// webimhandler.getBigMember(roomInfo.roomID, getMemberInfo, {
			// 	success: function (req) {
			// 		// console.log(req)
			// 		// console.log('进入房间调用获取群信息的')
			// 		// 将群总人数暴露到mlvbliverroomview.js
			// 		options.success && options.success({
			// 			roomID: roomInfo.roomID,
			// 			roomCreator: roomInfo.pushers[0].Member_Account,
...
function getGroupMember(options) {
	// 实际获取列表信息是在getMemberInfo里
	webimhandler.getBigMember(options.data.roomID, getMemberInfo, {
		success: function (req) {
			options.success && options.success({
				MemberList: req.roomInfo.MemberList,
				MemberNum: req.roomInfo.MemberNum,
			});
		},
		fail: options.fail
	});
...
function getMemberInfo(options) {
	...
	roomInfo.MaxMemberNum = options.data[0].MaxMemberNum
	roomInfo.MemberList = options.data[0].MemberList
	roomInfo.MemberNum = options.data[0].MemberNum
	roomInfo.pushers = options.data[0].pushers
	options.callback.success && options.callback.success({
		roomInfo
	});
}
...
module.exports = {
	login: login, // 初始化
	loginRoom: loginRoom, //小程序初始登录房间和IM
	initRoom: initRoom, // 小程序已经登录，初始化房间信息
	logout: logout, // 结束初始化
	...
	getGroupMember: getGroupMember, // 获取群成员列表
	sendRoomTextMsg: sendRoomTextMsg, // 发送文本消息
	sendRoomLOVEMsg: sendRoomLOVEMsg, // 发送点赞消息
	sendRoomCustomMsg: sendRoomCustomMsg, // 进房消息
