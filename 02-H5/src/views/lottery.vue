    ...
    <div class="lottery" v-if='isShow'>
      <div class='l_body' :style="{height:height+'px'}">
        ...
        <div class='l_turntable'>
          <img :src="imgPath+'order/l_t.png'" class='l_open' />
          <div class="canvas-container">
            <div class="canvas-content">
              <img :src="imgPath+'order/l_t_01.png'" class="canvas-element" />
          ...
          <div @click="startLottery" class="canvas-btn">
            <img :src="imgPath+'order/l_03.png'" style='width:1.75rem;height:2.02rem;' />
          </div>
        </div>
      </div>
      <div class='l_info'>
  ...
  methods: {
    ...
    winPro(e, listid) {
      ...
      if (this.allowbind) {
        ...
        addCarofGift(info).then(res => {
          if (res.isSuccess) {
            that.$router.togo({
              path: '/sureOrder?id=' + id + '&addType=3&proType=exchange&genre=1'
            })
          } else {
            that.allowbind = true
            Toast({
        ...
        if (that.times > 0) {
          ...
          if (that.btnDisabled === '') {
            ...
            if (this.$store.state.home.isbind === 'false' || this.$store.state.home.isbind === false) {
              ...
              var animeStart = anime({
                targets: el,
                rotate: {
                  value: 360 * 8 - 16 * (360 / 8),
                  duration: 5000,
                  easing: 'linear'
                },
                loop: false
