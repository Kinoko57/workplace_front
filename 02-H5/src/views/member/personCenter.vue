<template>
  <div>
    ...
    <div class='p_center'>
      <div class='pc_body'>
        ...
        <div class='pc_info'>
          ...
          <div class='info_attList' @click="edit('myIntersted','/myIntersted?visit=1','')">
            <div class='info_att'>
              <span v-show="personInfo.tag_ids!=''" v-for='(item,index) in personInfo.tag_list' :key='index'>{{item.name}}</span>
            </div>
            <i class='iconfont icon-youjiantou1'></i>
          </div>
        </div>
      ...
      <div class='exit_btn'>
        <mt-button form-type='submit' size='normal' @click='exit'>退出登录</mt-button>
      </div>
      <!-- 弹出框 -->
...
export default {
  ...
  data() {
    return {
      ...
      avatarUrl: process.env.FilE_URL + 'com/pc_avatar.png',
      hiddenavatar: true,
      personInfo: [],
      h_title: '个人信息',
      aweixin: ''
