        ...
        <div class="e-calendar-monthday">
          <transition :name="fadeXType">
            <div :key="showDate.monthStr" class="e-calendar-monthday-wrapper">
              ...
              <span v-for="(day,index) in row" :key='index' class="e-calendar-monthday-row-day" @click="selectDay(day)" :class="{'active': day.selected,'footstyle': day.hasfoot, 'disabled': day.disabled, 'pointer': day.value !== ''}">
    ...
    rows() {
      ...
      for (let i = 1; i <= months; i += 1) {
        ...
        if (i === 1 && weekValue !== 1) {
          ...
          this.addRowDayValue(row, i, this.footDays.includes(i))
        } else {
          this.addRowDayValue(row, i, this.footDays.includes(i))
  ...
  methods: {
    ...
    getFoot(date) {
      ...
      getFootDays(info).then(res => {
        if (res.isSuccess) {
          if (res.data.time_list != null) {
            for (var i = 0; i < res.data.time_list.length; i++) {
              footDays.push(new Date(res.data.time_list[i] * 1000).getDate())
            }
            that.footDays = footDays
          }
    ...
    addRowDayValue(row, i, isfoot) {
      const value = { value: i, hasfoot: false }
    ...
    prevMonth() {
      ...
      // this.setfootNum([])
      this.footDays = []
      var data = this.showDate.year + '-' + this.keepDoubleDigit(this.showDate.month) + '-' + '01'
      this.getFoot(data)
