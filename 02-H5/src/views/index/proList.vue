  ...
  <div>
    ...
    <div class='type_bg'>
      ...
      <div class='fixed_top'>
        ...
        <div class="type_header">
          <div class='serach'>
            ...
            <mt-field placeholder="搜索喜欢的商品" v-model="searchkey" class='search_input' @change="searchList" :disableClear="true"></mt-field>
          </div>
        </div>
        <!-- 顶部菜单栏 -->
        ...
        <div class='type_fifter'>
          ...
          <span
            :class="(searchFifter[0].type_id!=''||searchFifter[0].deal_type!=''||searchFifter[0].price_range!='')?'fifter_sort icon_active ':'fifter_sort'"
            @click="fifterAll">筛选
      ...
      <!-- <mt-loadmore :bottom-method="loadMore" :bottom-all-loaded="loading" ref="loadmore" :auto-fill='false'
        :bottomDistance='60' class='red_list' :bottomPullText="'按住上拉加载'" :bottomDropText="'松开加载更多'" :bottomLoadingText="'加载中'" v-if='isShow'> -->
      <!--   -->
      <div class='red_list' v-infinite-scroll="loadMore" infinite-scroll-disabled="loading"
      ...
      <div class='list_bottom' v-if='total==proList.length&&proList.length>0'>—————— 我是有底线的 ——————</div>
      <div class='red_list_null' v-if='!hasData'>
        <!-- <span>暂无数据</span> -->
        <img :src='imgPath+"pro/null_1.png"'></img>
      </div>
      <div class='fifter_more_bg' @click='hiddenFifterBg' v-if='fifter'></div>
      <div :class="{'animated slideInRight fifter_more':fifter,'animated slideOutRight fifter_more':!fifter}"
        style="height: 100%" v-if='fifter'>
        <div class='fifter_price'>
          <span>价格区间</span>
          <div>
            <input placeholder='最低价' type='text' v-model="lowPrice" @blur='low' />
            <i class='iconfont icon-jian'></i>
            <input placeholder='最高价' type='text' v-model="highPrice" @blur='high' />
          </div>
        </div>
        <!-- <div class='fifter_other' style="{height:brandHeight+'px'}">
          <div class='fifter_info' v-for="(item,index) in fifterList" :key='index'>
            ...
            <div :class="item.isopen?'fifter_options':'fifter_options_hidden fifter_options'">
              <span v-for="(item_fifter,index) in item.fifteroptions" :key="index"
                :class="item_fifter.checked?'chose_text':''"
                @click='chosefifterList(item_fifter.id,item.fifter_id,index)'
                v-if='item_fifter.id!=0'>{{item_fifter.name}}</span>
            </div>
          </div>
        </div> -->
...
export default {
  ...
  mounted() {
    ...
    // var routeData = this.$route.query
    // if (routeData.id !== undefined) {
    //   this.chose_nav_id = routeData.id
    //   this.source = routeData.type
    // }
    // if (routeData.value !== undefined) {
    //   this.searchkey = routeData.value
    //   this.source = routeData.type
    // }
    // if (routeData.point !== undefined) {
    //   this.searchFifter[0].point = routeData.point
    //   this.searchFifter[0].quantity = ''
    // }
    // if (routeData.price !== undefined) {
    //   this.searchFifter[0].price = routeData.price
    //   this.searchFifter[0].quantity = ''
    // }
    // if (routeData.virtual_id !== undefined) {
    //   this.virtual_id = routeData.virtual_id
    // }
    // Indicator.open('加载中...')
    // 获取分类
    // this.getType()
  ...
  methods: {
    ...
    low() {
      ...
      // console.log(this.lowPrice.length)
      // console.log(typeof this.lowPrice !== 'number')
      // console.log(isNaN(this.lowPrice) === false)
      var regu = '^[ ]+$'
      var re = new RegExp(regu)
      if (re.test(this.lowPrice)) {
        this.lowPrice = ''
      }
      if (this.lowPrice != '') {
        if (this.lowPrice.length > 0 && (typeof parseFloat(this.lowPrice) !== 'number' || isNaN(parseFloat(this
          .lowPrice)))) {
          Toast({
            message: '价格只能为数字',
            duration: 1000
          ...
          this.lowPrice = ''
        } else if ((parseFloat(this.lowPrice) < 0)) {
          Toast({
            message: '价格必须大于等于0',
            duration: 1000
          })
    ...
    high() {
      var regu = '^[ ]+$'
      var re = new RegExp(regu)
      if (re.test(this.highPrice)) {
        this.highPrice = ''
      }
      if (this.highPrice != '') {
        if (this.highPrice.length > 0 && (typeof parseFloat(this.highPrice) !== 'number' || isNaN(parseFloat(this
          .highPrice)))) {
          Toast({
            message: '价格只能为数字',
            duration: 1000
          ...
          this.highPrice = ''
        } else if ((parseFloat(this.highPrice) < 0)) {
          Toast({
            message: '价格必须大于等于0',
            duration: 1000
          })
  ...
  activated() {
    if (window.localStorage.getItem('backDetail') == 'false') {
      console.log(this.$route.query)
      var routeData = this.$route.query
      if (routeData.id !== undefined) {
        this.chose_nav_id = routeData.id
