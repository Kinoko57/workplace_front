    ...
    <div class="edithelp_bg">
      ...
      <el-form
        ...
        <el-form-item label="标题：" prop="title">
          <el-input v-model.trim="editTransaction.title" style="width:400px" placeholder="请输入标题" />
        </el-form-item>
        <el-form-item label="内容：" prop="content" class="rich_input">
          ...
          <my-Editor ref="myQuillEditor" :items="editorConfig" :content="editTransaction.content" height="400" @on-content-change="getValue"/>
...
import { getTransaction, saveTransaction, editTransaction } from '@/api/common'
  ...
  methods: {
    ...
    getList() {
      ...
      getTransaction().then(res => {
        ...
        if (res.data.docking[0] !== false) {
          that.isAdd = false
          that.editTransaction.title = res.data.docking.title
          that.editTransaction.content = res.data.docking.content
          that.editId = res.data.docking.id
          console.log(that.editTransaction.content, 'fff')
          that.editTransaction.file = res.data.docking.file
          if (this.editTransaction.file !== undefined && that.editTransaction.file !== '') {
            that.fileList = JSON.parse(that.editTransaction.file)
          }
    ...
    getValue(e) {
      this.editTransaction.content = e.html
      this.editTransaction.content = e.replace(/(^&nbsp;)|(&nbsp;$)/g, '')
    },
    saveModel() {
      const that = this
      this.$refs['editTransaction'].validate((valid) => {
        if (valid) {
          if (that.filesArr.length > 0) {
            that.editTransaction.file = JSON.stringify(that.filesArr)
          }

          that.submitting = true
          console.log(that.editTransaction, that.filesArr, 'fffff1111111111111')
          if (this.isAdd) {
            saveTransaction(this.editTransaction).then(res => {
            ...
            console.log(that.editTransaction, that.editId, 'jh')
            that.editTransaction.id = that.editId
            editTransaction(that.editId, that.editTransaction).then(res => {
              console.log(res, 'lll')
              if (res.code === 200 || res.code === '200') {
                that.$message({
                  message: '事务对接表更新成功！',
    ...
    resetForm() {
      this.$refs['editTransaction'].resetFields()
