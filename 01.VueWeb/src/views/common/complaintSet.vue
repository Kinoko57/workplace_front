<template>
  <div>
    <div class="work_bg">
      <div class="fifter">
        <my-tabs
          :add="btnPermission('complainlevel_create')"
          :allow-fifter="false"
          :return-show="true"
          :active-name="'all'"
          :tabs-list="tabsList"
          ...
          <!-- <search-fifter :add="btnPermission('complainlevel_create')" :add-text="'创建投诉类型'" :return-show="true"  @returnAction="returnBack" /> -->
      </div>
      <div class="work_info">
        ...
        <el-table v-loading="loading" :data="tableData" :max-height="tableHeight" tooltip-effect="light" border>
          ...
          <el-table-column label="操作" align="center" min-width="140">
            <template slot-scope="scope">
              <el-button v-if="btnPermission('complainlevel_edit')" type="text" size="small" @click="showModel(scope.row,'edit')">编辑</el-button>
              <el-button v-if="btnPermission('complainlevel_delete')" type="text" size="small" @click="changeStates(scope.row)">作废</el-button>
              <el-button v-if="btnPermission('complainprocessnode_edit')" type="text" size="small" @click="showModel(scope.row,'set')">配置工作流</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <el-dialog :close-on-click-modal="false" :modal-append-to-body="false" :visible.sync="modelShow" :title="modelTitle" width="800px" class="dialogModel" @close="resetModel">
        ...
        <div v-if="modelShow&&modelType!='set'" class="type_info">
          <el-form
            ...
            <el-form-item label="类型名称：" prop="name">
              <el-input v-model="typeInfo.name" style="width:500px" placeholder="请输入投诉标题" />
            </el-form-item>
      ...
      <el-scrollbar v-if="modelShow&&modelType=='set'" class="fixed_scroll over_scroll">
        <div v-if="modelShow&&modelType=='set'" class="type_set">
          ...
          <el-form ref="typeSetForm" :model="typeSetForm" class="modelForm" label-width="88px" label-position="right">
...
export default {
  ...
  methods: {
    ...
    changeStates(event) {
      const that = this
      const msg = '投诉类型一旦作废不可恢复，确定要“作废”吗？'
      this.$confirm(msg, '提示', {
        ...
        if (flag) {
          this.$refs['typeSetForm'].validate((valid) => {
            if (valid) {
              ...
              const submitUrl = baseURL + '/oa/complainprocessnode-edit-' + this.workId + '.json'
              const config = {
                headers: {
