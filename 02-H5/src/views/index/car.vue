        ...
        <div class='car_list'>
          <div class='car_pro' v-for="(item,index) in carList" :key="index">
            ...
            <div class='pro_info'>
              ...
              <div class="line_table"><span v-if="item.goods.is_third>0" class="pro_price_third">{{item.goods.third_name}}</span>
              <span v-else class="pro_price_third">平台自营</span>
              <span v-if="item.goods.is_gift_zone==1" class="pro_price_gift">{{item.goods.gift_title}}</span></div>{{item.name}}</div>
              ...
              <div class='pro_spec'>
                ...
                <div class='pro_icon'>
                  <mt-button size="small" class='pro_btn' @click='lessNum(item.product_id,item.quantity)' :disabled="item.quantity==1||item.quantity==0">-</mt-button>
                  <input v-model='item.quantity' @blur='changeValue(item.product_id,item.quantity)' type='number' @focus="isInput" />
                  <mt-button size="small" class='pro_btn' @click='addNum(item.product_id,item.quantity)' :disabled="item.quantity>=99||item.quantity==item.store_usefull">+</mt-button>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class='pro_price'>
              <div style='display:inline-flex'>
                <div class='pro_flex' :hidden='item.deal_type==2'>
                  <span class='peo_nowprice' v-if='item.deal_type==1'>￥{{item.price_sell}}<span style="color:#ff4e79;font-size:0.23rem" v-if="isMember"> / 赚{{item.back_rebate}}</span></span>
              ...
              <div class='pro_flex'>
                <i class='iconfont icon-car' @click='addCar(item.product_id,item.deal_type,item.buy_count)'></i>
              </div>
            </div>
          </div>
        </div>
  ...
  methods: {
    ...
    checkAll() {
      ...
      if (!flag) {
        ...
        for (let j = 0; j < this.carList.length; j++) {
          ...
          if (this.carList[j].goods.is_third == 0) {
            selfPrice += Number(this.carList[j].price_cart) * 10000 * this.carList[j].quantity // 自营总额
          }
        }
        ...
        console.log(k)
        if (chosePro.length == this.carList.length) {
          this.allchose = true
          this.isChose = true
        ...
        this.difference = ((this.full_amount * 10000 - selfPrice) / 10000).toFixed(2)
    ...
    chosePro(event) {
      ...
      for (var j = 0, len = this.carList.length; j < len; j++) {
        if (this.carList[j].checked) {
          sum += Number(this.carList[j].point) * this.carList[j].quantity * 100
          priceSum += Number(this.carList[j].price_cart) * 10000 * this.carList[j].quantity // 勾选全品类总额
          if (this.carList[j].goods.is_third == 0) {
            selfPrice += Number(this.carList[j].price_cart) * 10000 * this.carList[j].quantity // 自营总额
          }
          chosePro.push(this.carList[j].product_id)
      ...
      for (var i = 0, len = this.carList.length; i < len; i++) {
        if (this.carList[i].product_id === pid) {
          if (value > this.carList[i].store_usefull) {
            ...
            this.upPro(pid, this.carList[i].store_usefull)
          } else {
      ...
      for (var i = 0, len = this.carList.length; i < len; i++) {
        if (this.carList[i].product_id === pid) {
          if (value == 1) {
            ...
            if (Number(value) > Number(this.carList[i].store_usefull)) {
              value = Number(this.carList[i].store_usefull)
            } else {
              value--
            }
            this.upPro(pid, value)
      ...
      for (var i = 0, len = this.carList.length; i < len; i++) {
        if (this.carList[i].product_id === id) {
          if (value <= 1) {
            ...
            if (Number(value) > 99) {
              if (Number(value) > Number(this.carList[i].store_usefull)) {
                if (Number(this.carList[i].store_usefull) > 99) {
                  Toast({
                    message: '该商品最多可购买99件~',
                    duration: 1000
                  })
                  this.carList[i].quantity = 99
                  this.upPro(id, 99)
                } else {
                  Toast({
                    message: '库存不足',
                  ...
                  // 判断最大库存数
                  this.carList[i].quantity = this.carList[i].store_usefull
                  num = this.carList[i].store_usefull
                  if (num > 0) {
                    this.upPro(id, num)
                  }
                }
          ...
          if (res.num !== undefined) {
            ...
            for (var i = 0; i < that.carList.length; i++) {
              if (that.carList[i].product_id === id) {
                that.carList[i].quantity = res.num
              }
            }
            // 当商品被选中，更改数量时更新总数
          }
    ...
    upspec() {
      ...
      for (var j = 0, len = this.carList.length; j < len; j++) {
        if (this.carList[j].checked) {
          sum += Number(this.carList[j].point) * this.carList[j].quantity * 100
          priceSum += Number(this.carList[j].price_cart) * 10000 * this.carList[j].quantity
          if (this.carList[j].goods.is_third == 0) {
            selfPrice += Number(this.carList[j].price_cart) * 10000 * this.carList[j].quantity
          }
        }
      }
      this.scoreNum = sum / 100
      ...
      getCarListByCar().then(res => {
        ...
        if (res.isSuccess) {
          ...
          if (res.data.rows.length > 0) {
            ...
            var choseList = []
            console.log(that.$store.state.home)
            if (that.$store.state.home.carpro.length > 0) {
              choseList = that.$store.state.home.carpro
            }
            console.log('已选择的商品数量' + choseList)
                ...
                for (let c = 0; c < res.data.rows.length; c++) {
                  if (choseList[i] === res.data.rows[c].product_id) {
                    ...
                    priceSum +=
                        ...
                        10000 *
                        res.data.rows[c].quantity
                    if (res.data.rows[c].goods.is_third == 0) {
                      selfPrice += Number(res.data.rows[c].price_cart) * 10000 * res.data.rows[c].quantity
                    }
                  }
                }
              }
            ...
            that.difference = (
              (res.data.free_freight * 10000 - selfPrice) /
                10000
            ).toFixed(2)
...
<style scoped lang="less">
  ...
  .pro_info .pro_title .pro_price_third{
    display: table-cell;
  ...
  .pro_info .pro_title .pro_price_gift{
  // height: 24/100rem;
  // line-height: 24/100rem;
  line-height: normal;
  font-size: 20/100rem !important;
  color: #fff !important;
  background: #ff4e79;
  border: 2/100rem solid #ff4e79;
  border-radius: 5/100rem;
