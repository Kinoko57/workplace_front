<template>
  <div>
    <div class="find_wraper">
      <div class="find_top">
        ...
        <div class="find_top_content">
          <span class="find_top_content_span">质量件编号：<span
              class="style_color">{{ detailInfo.complainNumber }}</span></span>
          <span class="find_top_content_span">质量件类型：<span class="style_color">{{ detailInfo.processName }}</span></span>
          <span class="find_top_content_span">投诉日期：<span class="style_color">{{ detailInfo.createdDate }}</span></span>
        </div>
        <div class="find_top_content">
        ...
        <div v-if="viewType=='waitdeal'||viewType=='finishdeal'" class="find_top_btn">
          <el-button v-if="detailInfo.dealstatus==2" plain size="mini" icon="el-icon-edit-outline"
            @click="changeInfo('follow')">添加回访</el-button>
          <el-button :disabled="detailInfo.dealstatus==2||detailInfo.lowerstatus!=1" plain size="mini"
            icon="el-icon-edit-outline" @click="changeInfo('allocation')">质量件分配</el-button>
          <el-button :disabled="detailInfo.dealstatus==2" plain size="mini" icon="el-icon-edit-outline"
            @click="changeInfo('handling')">质量件处理</el-button>
          <el-button v-if="detailInfo.status==4" plain size="mini" icon="el-icon-edit-outline" @click="changeStates()">
            强制驳回</el-button>
        </div>
      </div>
          ...
          <div class="find_center_tit">
            ...
            <div class="overflow_scroll_div">
              <table class="view_dialog view_dialog_self" cellpadding="0" cellspacing="0">
                <tr class="view_line">
                  <td class="line_title">质量件主题：</td>
                  <td class="second_line">{{ detailInfo.title }}</td>
                  <td class="line_title">质量件类型：</td>
                  <td class="second_line">{{ detailInfo.processName }}</td>
                  <td class="line_title">登记人：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.registrant)}">{{ detailInfo.registrant|textNull }}</span>
                  </td>
                </tr>
                <tr class="view_line">
                  ...
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.accountName)}">{{ detailInfo.accountName|textNull }}</span>
                  </td>
                  <td class="line_title">提交人部门：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.organize_name)}">{{ detailInfo.organize_name|textNull }}</span>
                  </td>
                  <td class="line_title">提交人岗位：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.groupName)}">{{ detailInfo.groupName|textNull }}</span>
                  </td>
                </tr>
                <tr class="view_line">
                  <td class="line_title">商品名称：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.goods_name)}">{{ detailInfo.goods_name|textNull }}</span>
                  </td>
                  <td class="line_title">商品数量：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.goods_num)}">{{ detailInfo.goods_num|textNull }}</span>
                  </td>
                  <td class="line_title">联系人：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.contact_man)}">{{ detailInfo.contact_man|textNull }}</span>
                  </td>
                </tr>
                <tr class="view_line">
                  <td class="line_title">联系电话：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.contact_phone)}">{{ detailInfo.contact_phone|textNull }}</span>
                  </td>
                  <td class="line_title">联系地区：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.areasName)}">{{ detailInfo.areasName|textNull }}</span>
                  </td>
                  <td class="line_title">联系地址：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.contact_address)}">{{ detailInfo.contact_address|textNull }}</span>
                  </td>
                </tr>
                <tr class="view_line">
                  ...
                  <td class="second_line">
                    <span
                      v-if="detailInfo.registrantDate!='0000-00-00 00:00:00'&&detailInfo.registrantDate!=''">{{ detailInfo.registrantDate }}</span>
                    <span v-else style="color:#999">暂无数据</span>
                  </td>
                  <td class="line_title">投诉人姓名：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainUserName)}">{{ detailInfo.complainUserName|textNull }}</span>
                  </td>
                  <td class="line_title">投诉人电话：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainUserPhone)}">{{ detailInfo.complainUserPhone|textNull }}</span>
                  </td>
                </tr>
                <tr class="view_line">
                  ...
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainUserWx)}">{{ detailInfo.complainUserWx|textNull }}</span>

                  </td>
                  <td class="line_title">投诉人客服号：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainService)}">{{ detailInfo.complainService|textNull }}</span>
                  </td>
                  <td class="line_title" />
                  <td class="second_line" />
                </tr>
                <tr class="view_line">
                  ...
                  <td class="second_line" colspan="5">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainReason)}">{{ detailInfo.complainReason|textNull }}</span>
                ...
                <tr class="view_line">
                  ...
                  <td class="second_line" colspan="5">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.content)}">{{ detailInfo.content|textNull }}</span>
                ...
                <tr class="view_line">
                  ...
                  <td colspan="5" class="second_line">
                    <div v-for="(item,index) in detailInfo.file" :key="index">
                      <a v-show="detailInfo.file!=''&&item.type!=undefined&&item.type=='file'"
                        :href="detailInfo.url + item.url" download=""
                        style="display:block;font-size: 12px;max-width: 450px;width: auto;"
                        target="_blank">{{ item.name }}</a>
                      <span v-show="detailInfo.file!=''&&item.type!=undefined&&item.type=='img'"
            ...
            <div class="overflow_scroll_div">
              <table class="view_dialog view_dialog_self" cellpadding="0" cellspacing="0">
                <tr class="view_line">
                  ...
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainToUserName)}">{{ detailInfo.complainToUserName|textNull }}</span>
                  </td>
                  <td class="line_title">被投诉人电话：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainToUserPhone)}">{{ detailInfo.complainToUserPhone|textNull }}</span>
                  </td>
                  <td class="line_title">被投诉人微信：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainToUserWx)}">{{ detailInfo.complainToUserWx|textNull }}</span>
                  </td>
                </tr>
                <tr class="view_line">
                  ...
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainToService)}">{{ detailInfo.complainToService|textNull }}</span>
                  </td>
                  <td class="line_title">被投诉人团队：</td>
                  <td class="second_line">
                    <span
                      :class="{null_data_style:nullStyle(detailInfo.complainToUserBelongTeam)}">{{ detailInfo.complainToUserBelongTeam|textNull }}</span>
                  </td>
                  <td class="line_title" />
                  <td class="second_line" />
                </tr>
                ...
                <el-timeline-item v-for="(item,index) of followInfo" :key="index" placement="top" type="primary">
                  <el-card>
                    <div class="follow_name">
                      <div> <img src="@/assets/common/follow_user.png" alt=""
                          srcset=""><span>{{ item.accountName }}</span></div>
                    ...
                    <div class="follow_bto">
                      <div class="follow_file">附件：<a v-if="item.file[0].name!=''" :title="item.name"
                          :href="fileUrl+item.file[0].url" download="" target="_blank"
                ...
                <div class="operate_con">
                  ...
                  <span class="span_cont">
                    ...
                    <span v-if="item.action=='audit'"><span>由</span> <span
                      ...
                      <span v-if="item.extra!=''">，备注：</span><span v-if="item.extra!=''"
                    ...
                    <span v-if="item.action=='dispose'"><span>由</span> <span
                      ...
                      <span>附件：</span>
                      <span>
                        <a v-for="(item,index) of item.comment.file" v-show="item.name!=''" :key="index"
                          :title="item.name" :href="fileUrl+item.url" download="" target="_blank"
    ...
    <el-dialog :close-on-click-modal="false" :modal-append-to-body="false" :visible.sync="modelShow" :title="modelTitle"
      width="600px" class="dialogModel" @close="resetModel">
          ...
          <div class="approval_form" style="padding-top:20px">
            ...
            <div v-if="modelType=='allocation'" class="view_comments" style="align-items:center;">
              ...
              <el-select v-model="doUser" placeholder="请选择下级" style="width:250px">
                <el-option v-for="item in userInfo" :key="item.account" :label="item.realname" :value="item.account"
                  style="width:250px" />
              </el-select>
            </div>
            ...
            <div v-if="modelType=='handling'" class="view_comments" style="align-items:center;">
              ...
              <el-upload ref="upload" :on-preview="handlePreview" :on-exceed="handleExceed"
                :before-upload="beforeFileUpload" :on-remove="handleRemove" :on-success="handleSuccess" :name="'files'"
      ...
      <el-scrollbar v-if="modelType=='follow'&&modelShow" class="fixed_scroll over_scroll">
        <el-form ref="followComplaintForm" :model="followComplaintForm" class="modelForm" label-width="120px"
          ...
          <el-form-item label="回访方式：" class="red_star_wraper">
            ...
            <el-date-picker v-model="followComplaintForm.thisDate" type="datetime" style="width:250px"
          ...
          <el-form-item label="回访内容：" class="red_star_wraper">
            <i class="red_star">*</i>
            <el-input v-model.trim="followComplaintForm.content" :rows="4" maxlength="500" type="textarea"
              autocomplete="off" style="width:500px" placeholder="请输入内容,最大长度为500" />
          </el-form-item>
          <el-form-item label="下次回访时间：" class="red_star_wraper">
            <i class="red_star" style="position: absolute;left: -120px;">*</i>
            <el-date-picker v-model="followComplaintForm.nextDate" type="datetime" format="yyyy-MM-dd HH:mm:ss"
              value-format="yyyy-MM-dd HH:mm:ss" style="width:500px" placeholder="请选择回访时间" @change="dateChange" />
          </el-form-item>
          <el-form-item label="附件：" prop="file" class="red_star_wraper">
            <i class="red_star" style="position: absolute;left: -63px;">*</i>
            <el-upload ref="upload" :on-preview="handlePreview" :on-exceed="handleExceed" :before-upload="beforeUpload"
              :on-success="handleSuccess" :on-remove="handleRemove" :on-change="uploadChange" :limit="1" :name="'files'"
              :action="uploadUrl" :file-list="fileList" style="width:500px" class="upload-demo" multiple>
              <el-button size="small" type="primary">点击上传</el-button>
        ...
        viewComplaintPageDetail(this.detailId).then(res => {
          if (res.code === 200) {
            ...
            that.carStep.unshift({
              name: '提交投诉',
              auditStatus: '2',
              auditer: that.detailInfo.accountName,
              auditDate: that.detailInfo.createdDate,
              auditRemark: ''
            })
            ...
            if (that.detailInfo.file !== undefined && that.detailInfo.file !== '' && that.detailInfo.file !==
              'null' &&
            ...
            this.$refs['followComplaintForm'].validate((valid) => {
              if (valid) {
                ...
                if (that.follow_type == 'add') {
                  ...
                  followEdit(that.follor_id, this.followComplaintForm).then(res => {
                    console.log(res)
                    that.submitting = false
                    if (res.code === 200 || res.code === '200') {
                      this.$message({
