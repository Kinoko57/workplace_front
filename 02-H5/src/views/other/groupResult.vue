    ...
    <div class='result_group'>
      ...
      <div class='group_order'>
          ...
          <div class='group_person'>
            <div class='group_img' v-for='(item,index) in groupPeople' :key='index'>
              <div v-if="index<4">
                ...
                <img :src="imgPath+'com/pc_avatar.png'" v-if="item.image==null||item.image==''" />
              </div>
            </div>
        ...
        <div class='sale_btn' :hidden='GroupOrder.status==-1||GroupOrder.startnum<=groupPeople.length'>
          ...
          <mt-button @click='zipImg'>邀请好友参团</mt-button>
...
export default {
  ...
  methods: {
    ...
    shareGroup(img) {
      ...
      // 正式gh_a0cfbcaf8fa2 测试 gh_05718de312bd
      let shareUrl = '/pages/groupResult/groupResult?type=payshare&orderId=' + this.id + '&uid=' + storage.getItem('token').split(';')[0].split('=')[1]
      let linkURL = ''
      ...
      let msg = {
        type: 'miniProgram',
        title: '[邀请参团]' + that.GroupOrder.name,
        thumbs: [img],
