  ...
  <div>
    ...
    <div class='scan' v-if='isShow'>
      <div class='scan_body'>
        ...
        <!-- <swiper indicator-dots="true" autoplay='true' indicator-color='rgba(0, 0, 0, .3)' indicator-active-color='#ff4e79'
          interval='3000'>
    ...
    <div class='scan_win' v-if='winhidden' catchtouchmove="ture">
      <img :src="imgPath+'pro/win_bg_03.png'"/>
      ...
      <div class='win_text' @click='goCou'>
        <img :src="imgPath+'member/r_btn.png'"/>
      </div>
    </div>
    <div class='gift_box' @click='openWin' v-if='winhidden==false&&winType!=-1&&isShow'>
      <img :src="imgPath+'pro/gift.png'" class='animated pulse infinite' />
    </div>
  </div>
</template>
        ...
        if (res.isSuccess) {
          ...
          axios.get(that.winUrl, param, config).then(response => {
            ...
            if (response.data.isSuccess) {
              ...
              that.winType = -1
              that.errTips = response.data.msg
              that.winhidden = false
              if (response.data.isget == false) {
                that.goLogin('/myGift')
              }
            }
          })
