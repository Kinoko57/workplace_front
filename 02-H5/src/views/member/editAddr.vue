    ...
    <div class='editAddr'>
      <div class='ea_body'>
        <div class='form_body'>
          ...
          <div class='saveInfo'>
            <mt-button :disabled='is_disabled' @click="formSubmit" >保存</mt-button>
          </div>
        </div>
      </div>
...
import addressArr from '@/store/area.json'
import {
  InfiniteScroll,
  Indicator,
  Button,
...
export default {
  ...
  created() {
    for (let i in addressArr) {
      let cityObj = {}
      for (let j in addressArr[i].city) {
        cityObj[addressArr[i].city[j].name] = addressArr[i].city[j].area
      }
      this.myaddress[addressArr[i].name] = cityObj
    }
  },
  mounted() {
    this.$nextTick(() => { // vue里面全部加载好了再执行的函数  （类似于setTimeout）
      this.myAddressSlots[0].defaultIndex = 0
      this.myAddressSlots[0].values = Object.keys(this.myaddress)
    ...
    if (options.type === 'new' || options.type === 'choose') {
      ...
      let sureData = this.$store.state.home.choseAddr
      this.beforeData = sureData
      this.pid = this.staticInfo.checked
    ...
    } else if (options.type === 'choose_edit') {
      this.form = {
        $dirty: false,
        $invalidMsg: '',
        addr: false,
    ...
    formSubmit: function (e) {
      ...
      if (this.form.name !== false) {
        ...
        // 订单详情修改地址
          ...
          console.log(addinfo)
          setDefaultAddr(addinfo).then(res => {
            if (res.isSuccess) {
              Toast({
                message: '操作成功',
                iconClass: 'iconfont icon-dui1',
              ...
              if (that.addInfo.is_default) {
                ...
                if (that.addFrom === 'new') {
                  ...
                  let info = {
                    coupon_id: that.beforeData.couId,
                    coupon_code: that.beforeData.choseId,
                    checked: that.pid,
                    shipmethod_id: that.beforeData.ship_id,
                    cart_type: that.beforeData.addType,
                    type: that.beforeData.orderType,
                    address_id: addinfo.id
                  }
                  getPriceByTrack(info).then(req => {
                    if (req.isSuccess) {
                      console.log(that.beforeData)
                      let data = that.beforeData
                      data.amount = req.amount
                      let cou = ''
                      if (that.beforeData.choseId === '') {
                        cou = '选择优惠券'
                      } else {
                        cou = '已减' + req.coupon_discount + '元'
                      }
                      data.chooseAdd = addinfo.id
                      data.addisRevise = true
                      data.couInfo = cou
                      data.couValue = req.coupon_discount
                      data.couCode = req.cart_params
                      data.staticAdd = []
                      data.staticAdd.push(addinfo)
