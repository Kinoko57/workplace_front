<template>
  <div style='background:#0d4dae'>
    ...
    <div class='my_vip' v-if="isShow" :style="{height:vipHeight+'rem'}">
      ...
      <div class='vip_body'>
        <img :src='imgPath+"member/b_top.png"' class="list_img" />
        <div class='vip_list_bg'>
          <div class='vip_list' v-infinite-scroll="loadMore" infinite-scroll-disabled="loading"
        ...
        <img :src='imgPath+"member/b_bottom.png"' class="list_img" />
      </div>
    </div>
    ...
    getList: function () {
      ...
      getBirthdayList(info).then(res => {
        ...
        if (res.isSuccess) {
          ...
          that.loading = false
          that.$nextTick(() => {
            that.vipHeight = document.querySelector('.vip_body').offsetHeight / 50 + 4.7
          })
      ...
      setTimeout(() => {
        ...
        getBirthdayList(info).then(res => {
          ...
          if (res.isSuccess) {
            ...
            that.vipList = that.vipList.concat(res.data.list.rows)
            that.loading = false
          } else {
            Toast({
              message: res.msg,
